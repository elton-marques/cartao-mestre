# Deploy — Cartão Mestre (eltonmarques.com/cartaomestre)

Documenta como o dashboard ficou publicado na VPS Oracle, pra dar pra
reproduzir/atualizar de outro PC sem precisar redescobrir tudo.

> Este projeto foi separado do hub pessoal (`eltoncastor/cartao_mestre`
> virou `elton-marques/eltonmarques-site`) — os dois seguem publicados na
> mesma VPS, como serviços irmãos sob o mesmo domínio, mas agora em
...
Dois serviços deste repo rodando só em `127.0.0.1` (nunca expostos
...
eltonmarques.com/ (catch-all)   → http://localhost:8081    (hub — outro repo)
...
repo. O login (servido em `eltonmarques.com/login/`) segue o mesmo
raciocínio: `../design-system/` (um nível abaixo, como o app).
...
(`/var/www/hub/` também existe na mesma VPS, mas pertence ao outro repo —
ver `elton-marques/eltonmarques-site`.)
